"""
This program scrapes the Zarrar Khuhro columns from dawn.com website.
...
req = Request( "https://www.dawn.com/authors/1915/zarrar-khuhro", headers = { "User-Agent": "Mozilla/5.0" } )
...
numberOfRowsReturned = cursor.execute( "SELECT column_id FROM zarrar_khuhro" )
    ...
    for article in articles:
        ...
        if len( results ) != 0:
            if column_data_id not in results_list:
                query = 'INSERT INTO zarrar_khuhro (column_id, column_title, column_publish_date) VALUES (%s, %s, %s)'
                ...
                commandLine = 'echo "' + email_body + '" | mail -s "New Zarrar Khuhro Column" "' + NOTIFY_EMAIL + '"'
